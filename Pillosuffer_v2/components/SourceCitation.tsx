...
export default function SourceCitation({ source }: Props) {
  ...
  return (
    <div className="flex items-center gap-2 mt-2.5 pt-2.5 border-t border-white/[0.04]">
      <span className={`dot ${cfg.dot} flex-shrink-0`} />
      <span className="text-[10px] tracking-wider text-zinc-400 font-semibold uppercase">
        {cfg.label}
      </span>
      <span className="text-[10px] text-zinc-500 leading-relaxed truncate" title={cfg.tooltip}>
        · {source}
      </span>
    </div>
  )
