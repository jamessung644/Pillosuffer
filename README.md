...
- Gemini 3.8 Flash는 음식 이미지 인식에만 사용합니다. 현재 상호작용 조회에는 LLM을 사용하지 않습니다.

## 현재 운영 방식 (2026-09-11)

- 운영 DB에서 상호작용 테이블 부재가 확인되어 AI 답변 생성을 먼저 중단했습니다. 사용자 승인 후 로컬 JSON·CSV가 일치하는 원문 2,512건을 Supabase에 복구했습니다.
- 상호작용 화면은 **원문 검색 전용**입니다. AI 일반 지식, 임상 합의 추측, AI 출처를 DB 출처로 바꾸는 후처리, 자동 안전 등급 생성을 제거했습니다.
- 원문에 명시된 회피·복용 지침을 제한된 고정 번역 규칙으로 연결해 **빨강: 피하세요 / 초록: 함께 복용 가능 / 주황: 확인 필요**로 표시합니다. 한국어 안내를 먼저 보여주고 원문·출처는 펼쳐보기로 제공합니다. 단순 검색 성공이나 자료 없음은 초록색의 근거가 아닙니다. 조회 오류나 불완전한 데이터셋은 HTTP 503으로 중단합니다.
- 이 방식은 키워드 기반 검색이며 벡터 검색이나 LLM 설명 생성이 아닙니다. 약품 성분은 공식 제품허가 정보와 제품명 일치를 확인한 경우에만 검색에 사용합니다. 검색어 일치는 임상적 안전성 검증을 의미하지 않습니다.
- [복구·검증·제한 사항](docs/evidence-recovery.md)을 확인하세요. 아래의 LLM/RAG 구성과 성능 수치는 **발표 당시 설계·과거 평가 기록**이며 현재 서비스의 동작이나 안전성 보장이 아닙니다.

이하 README는 다음 발표 자료를 기반으로 재구성한 과거 프로젝트 기록입니다.
...
| LLM | 발표 당시 검색 근거를 바탕으로 문장 생성. 2026-09-11 이후 상호작용 조회에서는 제거 |
