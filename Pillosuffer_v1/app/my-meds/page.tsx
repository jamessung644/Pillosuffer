    ...
    <div className="page-padding flex flex-col min-h-screen">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => router.push('/')}
          aria-label="홈으로"
          className="w-11 h-11 shrink-0 rounded-2xl bg-white shadow-sm flex items-center justify-center text-gray-600"
        >
        ...
        {groups.length > 0 && (
          <button
            ...
            aria-label="저장된 약 전체 삭제"
            title="저장된 약 전체 삭제"
            className="w-11 h-11 shrink-0 flex items-center justify-center text-red-700 bg-red-50 rounded-xl"
          >
      ...
      {groups.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-6">
          ...
          <Link href="/manual" className="mt-3 px-4 py-3 text-blue-700 font-semibold">약 이름 직접 입력</Link>
        </div>
      ) : (
        <>
                ...
                <section key={group.id}>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-2 mb-2 px-1">
                    <div className="flex flex-wrap items-center gap-2 min-w-0">
                      ...
                      {relative && (
                        <span className="text-sm text-gray-600 flex-shrink-0">{absolute}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0 ml-auto">
                      ...
                      <button
                        ...
                        className="text-sm text-red-700 px-3 py-3 rounded-lg bg-red-50 active:bg-red-100 font-medium"
                      >
                        묶음 삭제
                      </button>
                    </div>
