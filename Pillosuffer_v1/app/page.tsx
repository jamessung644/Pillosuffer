...
function Tile({ href, icon, title, color }: TileProps) {
  return (
    <Link
      href={href}
      className={`flex flex-col items-center justify-center gap-3 rounded-2xl p-4 min-h-[144px] text-white shadow-sm active:scale-[0.97] transition-transform ${color}`}
    >
      <Icon name={icon} size={38} stroke={2.2} />
      <span className="text-lg text-center leading-6 font-bold">{title}</span>
    </Link>
  )
}
  ...
  return (
    <div className="page-padding flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="mb-5 pt-1">
        <h1 className="text-[34px] leading-tight font-black text-gray-900">뭐무꼬</h1>
