...
export default function ProfilePage() {
  ...
  function handleSave() {
    if (profile.age && (!/^\d+$/.test(profile.age) || Number(profile.age) > 130)) {
      setError('나이는 0~130 사이의 정수로 입력해 주세요.')
      return
    }
    try {
      saveProfile(profile)
      ...
      setError('정보를 저장하지 못했습니다. 기기의 저장 공간과 브라우저 설정을 확인해 주세요.')
    }
  }
    ...
    <div className="page-padding flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => router.push('/')}
          aria-label="홈으로"
          className="w-11 h-11 shrink-0 rounded-2xl bg-white shadow-sm flex items-center justify-center text-gray-600 active:bg-gray-200"
        >
...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <label className="block min-w-0">
      <span className="block text-base font-semibold text-gray-700 mb-1.5">{label}</span>
      {children}
      {hint && <span className="block text-sm text-gray-600 mt-1.5">{hint}</span>}
    </label>
  )
}
