...
export default function ManualPage() {
  ...
  function add() {
    ...
    const session: ScanSession = readScanSession(raw)
  ...
  return (
    <div className="page-padding flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="mt-auto pt-8 space-y-3">
        <button onClick={add} disabled={!name.trim()} className="w-full py-4 bg-blue-600 text-white text-lg font-bold rounded-2xl active:bg-blue-700 transition-colors disabled:opacity-50">
          약 추가하기
        </button>
...
function Field({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  ...
  return (
    <div className="min-w-0">
      <label htmlFor={id} className="block text-base font-bold text-gray-800 mb-1.5">{label}</label>
      <input
        id={id}
