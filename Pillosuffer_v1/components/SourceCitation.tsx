...
export default function SourceCitation({ source }: Props) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-2 pt-2 border-t border-gray-200/60">
      <span className={`w-1.5 h-1.5 rounded-full inline-block flex-shrink-0 ${cfg.dot}`} />
      <span className="text-xs text-gray-600 font-semibold" title={cfg.tooltip}>
        {cfg.label}
      </span>
