...
export default function DrugCard({ drug, index, onUpdate, onDelete, onEditingChange }: Props) {
  ...
  const infoRequest = useRef<AbortController | null>(null)

  useEffect(() => { setDraft(drug); setEditing(false) }, [drug])
  useEffect(() => {
    if (!editing) return
  ...
  useEffect(() => {
    setInfo(null); setPermit(null); setInfoOpen(false); setInfoState('idle')
    return () => infoRequest.current?.abort()
  }, [drug.name])
  ...
  async function toggleInfo() {
    ...
    infoRequest.current = controller
    try {
      const res = await fetch(apiUrl(`/api/easy-drug?name=${encodeURIComponent(drug.name)}`), { signal: controller.signal })
      if (!res.ok) throw new Error('Drug information unavailable')
      ...
      if (!controller.signal.aborted) setInfoState('error')
    }
  }

  function save() {
    if (!draft.name.trim()) return
    onUpdate(index, { ...draft, name: draft.name.trim() })
    setEditing(false)
  }
      ...
      <div className="bg-white rounded-2xl border border-blue-200 p-4 shadow-sm">
        <div className="space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={save} disabled={!draft.name.trim()} className="flex-1 py-3 bg-blue-600 text-white rounded-xl text-base font-bold active:bg-blue-700 disabled:opacity-50">저장</button>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        <p className="text-lg font-bold text-gray-900 min-w-0 break-words">{drug.name}</p>
      </div>
