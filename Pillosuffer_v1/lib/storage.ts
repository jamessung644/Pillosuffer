...
import { isRecord, parseStoredJson, readDrugs } from './validation'
...
export function getProfile(): UserProfile {
  ...
  try {
    ...
    for (const key of ['name', 'age', 'allergies', 'conditions'] as const) {
      if (typeof value[key] === 'string') profile[key] = value[key]
    }
    if (value.gender === 'male' || value.gender === 'female' || value.gender === 'other') profile.gender = value.gender
    return profile
  } catch {
    return {}
  }
...
export function getMedGroups(): MedGroup[] {
  ...
  try {
    ...
    if (Array.isArray(value)) {
      return value.filter(g => isRecord(g) && typeof g.id === 'string' &&
        typeof g.scannedAt === 'string' && (g.source === 'scan' || g.source === 'manual') && Array.isArray(g.drugs))
        .map(g => ({ id: g.id, scannedAt: g.scannedAt, source: g.source, drugs: readDrugs(g.drugs) }))
  ...
  try {
    ...
    const legacyDrugs = readDrugs(parseStoredJson(legacy))
